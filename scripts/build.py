...
def logo_svg()->str:
    return ('<svg viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
            '<rect width="40" height="40" rx="10" fill="#0069b2"/>'
            '<path d="M22.5 7 11 22h7.5L16 33 29 17h-7.5z" fill="#fff"/>'
            '</svg>')
...
def footer()->str:
    o=CONFIG['operator']
    contact=''
    if CONFIG['phone']:contact+='<br><b>상담전화</b> '+E(CONFIG['phone'])
    if CONFIG['email']:contact+=' &nbsp; <b>이메일</b> '+E(CONFIG['email'])
    if o['publicAddress']:contact+='<br><b>주소</b> '+E(o['publicAddress'])
    ...
    <div class="business-info"><b>상호</b> {E(o['name'])} &nbsp; <b>대표</b> {E(o['representative'])} &nbsp; <b>사업자등록번호</b> {E(o['registrationNumber'])}{contact}<br>지역별 안내는 서비스 제공 지역의 구분이며, 각 지역의 지사·영업소 소재지를 뜻하지 않습니다.</div>
...
def home()->str:
    ...
    <section class="section"><div class="wrap faq-grid"><div class="faq-intro">{eyebrow('FAQ')}<h2>자주 묻는 질문</h2></div>{faq()}</div></section>{cta()}</main>'''
...
def services()->str:
    ...
    <section class="section"><div class="wrap"><div class="section-head"><div>{eyebrow('PROCESS')}<h2>문의부터 착수까지</h2></div></div>{process()}</div></section>
    <section class="section section-soft"><div class="wrap faq-grid"><div class="faq-intro">{eyebrow('FAQ')}<h2>상주 위탁<br>자주 묻는 질문</h2></div>{faq()}</div></section>{cta(service='onsite')}</main>'''
...
def about()->str:
    ...
    return f'''<main id="main"><section class="page-hero bg-about"><div class="wrap">{breadcrumb([('/about/','회사 소개')])}{eyebrow('ABOUT US')}<h1>전기안전관리,<br>현장의 기준이 됩니다.</h1><p class="lead">{E(BRAND)}는 {AREA} 사업장의 전기안전관리자 상주선임·위탁과 직무고시 대행을 제공합니다. 자격을 갖춘 인력과 명확한 업무 기준으로 현장의 전기안전을 책임집니다.</p></div></section>
    <section class="section-tight"><div class="wrap"><div class="feature-grid"><div class="feature"><span class="num">01</span><h3>상주 위탁 전문</h3><p>전기안전관리자 상주선임·위탁을 주력으로, 신규 선임·업체 변경·직접고용 전환을 담당합니다.</p></div><div class="feature"><span class="num">02</span><h3>명확한 업무 기준</h3><p>포함 업무와 별도 업무를 계약 단계에서 구분해 운영 중 혼선을 없앱니다.</p></div><div class="feature"><span class="num">03</span><h3>다섯 권역 집중</h3><p>{AREA}에 집중해 현장 대응과 인력 운영의 안정성을 높입니다.</p></div><div class="feature"><span class="num">04</span><h3>비공개 상담</h3><p>고객사 정보와 문의 내용은 외부에 공개하지 않습니다.</p></div></div></div></section>
    <section class="section-tight section-soft"><div class="wrap article"><section class="content-block"><h2>사업자 정보</h2><div class="privacy-block"><dl><dt>상호</dt><dd>{E(o['name'])}</dd><dt>대표자</dt><dd>{E(o['representative'])}</dd><dt>사업자등록번호</dt><dd>{E(o['registrationNumber'])}</dd>{('<dt>주소</dt><dd>'+E(o['publicAddress'])+'</dd>') if o['publicAddress'] else ''}{('<dt>상담전화</dt><dd>'+E(CONFIG['phone'])+'</dd>') if CONFIG['phone'] else ''}{('<dt>이메일</dt><dd>'+E(CONFIG['email'])+'</dd>') if CONFIG['email'] else ''}</dl></div></section><section class="content-block"><h2>견적·계약·수행</h2><div class="light-panel"><h3>{E(p['name'])}</h3><p>확인된 등록 범위와 현장 조건에 따라 견적·계약·선임 및 업무 수행을 진행합니다.</p></div><p class="source-note">전기안전관리 업등록 기준: <a href="https://www.keea.or.kr/head/work/getWWO03R01R03.do" target="_blank" rel="noopener noreferrer">한국전기기술인협회 업등록 안내</a></p></section><section class="content-block"><h2>서비스 지역</h2><p>{AREA}의 시·군·구 사업장을 대상으로 합니다. 지역별 안내 페이지는 서비스 제공 지역의 구분이며, 각 지역의 지사 소재지를 뜻하지 않습니다.</p><div class="related-links">{a('/regions/','서비스 지역 전체 보기 <span aria-hidden="true">→</span>')}</div></section></div></section>{cta()}</main>'''

def privacy()->str:
    p=CONFIG['privacy'];live=not PREVIEW
    if not live:
        content='''<div class="draft-note">개인정보처리방침 초안입니다. 운영 개시 전 처리자·연락처·보유기간을 확정해 최종 본문으로 교체합니다.</div><section class="content-block" style="margin-top:32px"><h2>수집 항목과 목적</h2><p>견적 문의 응대와 현장 조건 확인을 위해 연락처, 현장 지역, 시설 유형, 문의 유형, 운영 희망 조건을 수집합니다. 담당자명·이메일·설비 정보·요청 사항은 선택 항목입니다.</p></section><section class="content-block"><h2>처리 방식</h2><p>문의는 공개 게시판이 아닌 비공개로 접수되며, 상담 목적 외에 사용하지 않습니다. 첨부파일은 받지 않으며, 주민등록번호 등 민감정보는 입력하지 마세요.</p></section><section class="content-block"><h2>운영 개시 전 확정할 내용</h2><ul class="check-list"><li>개인정보처리자와 개인정보 보호 연락 창구</li><li>문의 보유기간과 파기 절차</li><li>운영 사업자와 수행업체 사이의 위탁·제3자 제공 관계</li><li>열람·정정·삭제 등 권리 행사 절차</li></ul></section>'''
    else:
    ...
    return f'''<main id="main"><section class="page-hero"><div class="wrap">{breadcrumb([('/privacy/','개인정보처리방침')])}{eyebrow('PRIVACY')}<h1>개인정보처리방침</h1><p class="lead">문의 시 수집하는 정보와 처리 방식을 안내합니다.</p></div></section><section class="section-tight"><div class="wrap article">{content}</div></section></main>'''
...
def quote_page()->str:
    ...
    turnstile=f'<div class="cf-turnstile" data-sitekey="{E(CONFIG["form"]["turnstileSiteKey"])}" data-action="inquiry"></div>' if not PREVIEW else ''
...
def verify_production():
    ...
    if CONFIG['form']['enabled']:checks.append((CONFIG['form']['turnstileSiteKey'],'form.turnstileSiteKey'))
...
def main():
    ...
    (dist/'favicon.svg').write_text('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40"><rect width="40" height="40" rx="10" fill="#0069b2"/><path d="M22.5 7 11 22h7.5L16 33 29 17h-7.5z" fill="#fff"/></svg>',encoding='utf-8')
    ...
    fonts='<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.min.css"><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Poppins:wght@500;600&family=Montserrat:wght@300;400;500&display=swap">'
